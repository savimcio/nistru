...
Nistru is a Bubble Tea TUI editor with an in-proc + out-of-proc plugin system. The editor core lives in [internal/editor/](internal/editor/) ([model.go](internal/editor/model.go), [editor.go](internal/editor/editor.go), [editoriface.go](internal/editor/editoriface.go), [goeditor_adapter.go](internal/editor/goeditor_adapter.go), [palette.go](internal/editor/palette.go), [autosave.go](internal/editor/autosave.go)); the layered TOML config (user + project + env, threaded into editor and plugins) lives in [internal/config/](internal/config/); the binary entry point is [cmd/nistru/main.go](cmd/nistru/main.go); first-party in-proc plugins live under [internal/plugins/](internal/plugins/); the host lives in [plugin/](plugin/); the plugin SDK and harness in [sdk/plugsdk/](sdk/plugsdk/); examples in [examples/](examples/). The Charm stack is on v2 (`charm.land/bubbletea/v2`, `charm.land/lipgloss/v2`, `github.com/charmbracelet/x/exp/teatest/v2`); `x/ansi` stayed on `github.com/charmbracelet/x/ansi`. See [docs/testing.md](docs/testing.md) for the full testing strategy. This file is the fast rules.
...
- **Component** — drives `Model.Update` or `plugin.Host` directly, no `tea.Program` → [internal/editor/model_component_test.go](internal/editor/model_component_test.go), [plugin/host_test.go](plugin/host_test.go), [plugin/gaps_test.go](plugin/gaps_test.go).
- **E2E** — real `tea.Program` via `teatest`, behind `//go:build e2e` → [internal/editor/model_e2e_test.go](internal/editor/model_e2e_test.go).

See [docs/testing.md](docs/testing.md) for when each tier applies.

## When modifying X, update Y

- Touching [internal/editor/model.go](internal/editor/model.go) `Update()` / message handling → add or adjust a test in [internal/editor/model_component_test.go](internal/editor/model_component_test.go); if behavior-visible, also update [internal/editor/model_e2e_test.go](internal/editor/model_e2e_test.go) snapshots.
- Touching [internal/editor/model.go](internal/editor/model.go) `View()` / `renderStatusBar()` / layout → regenerate affected goldens under [internal/editor/testdata/golden/](internal/editor/testdata/golden/) (`go test -tags=e2e -update ./...`), **diff them by hand in the PR**, do not blanket-accept.
- Touching [internal/editor/palette.go](internal/editor/palette.go) (filtering, layout) → [internal/editor/palette_test.go](internal/editor/palette_test.go) unit coverage + visual regression via `internal/editor/testdata/golden/palette_*`.
- Touching [internal/editor/autosave.go](internal/editor/autosave.go) → extend [internal/editor/autosave_test.go](internal/editor/autosave_test.go); if `flushNow` shape changes, adjust the `nowFunc` seam or the component-test clock stub accordingly.
- Touching [internal/editor/editor.go](internal/editor/editor.go), [internal/editor/editoriface.go](internal/editor/editoriface.go), or [internal/editor/goeditor_adapter.go](internal/editor/goeditor_adapter.go) (Ctrl-shortcut interception, mode synthesis, adapter contract) → [internal/editor/editor_test.go](internal/editor/editor_test.go) for the `fakeEditor` test double and [internal/editor/goeditor_adapter_test.go](internal/editor/goeditor_adapter_test.go) for the adapter; Ctrl shortcuts must transit Normal mode before synthesising vim motions through the adapter.
...
- Location: [internal/editor/testdata/golden/](internal/editor/testdata/golden/) (`*.txt`).
